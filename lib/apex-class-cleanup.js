...
let createConnectionConfig = function(options) {
    let elementKeys = [
        'instanceUrl',
        'accessToken',
        'loginUrl',
...
let requiresLogin = function(options) {
    return 'accessToken' in options;
}

let login = function(options) {
    let config = createConnectionConfig(options);
    const connection = new jsforce.Connection(config);
    let result = {connection, options};

    if('metadataPollTimeout' in options) {
        connection.metadata.pollTimeout = options.metadataPollTimeout;
    }

    return
        ('accessToken' in options) // when accessToken supplied, no need to log in
        ?
